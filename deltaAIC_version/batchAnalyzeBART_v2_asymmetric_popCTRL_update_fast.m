...
for pt = fliplr(pts)
    ...
    elseif strcmp('TDlearnNeural',analysis)
        ...
        optimalAlphas = false; % true if want best learning rate, false to run gradients
         ...
         if optimalAlphas
             [TDdata(pt), ~, ~, outcomeAndCueHGs(pt)] = BART_bankpop_bhf_TDlearn2(ptID,whichTrialsLM,false); % if plt true then this plots high gamma examples.
         elseif ~optimalAlphas
            [TDdataGradients(pt), ~, ~, outcomeAndCueHGs(pt)] = BART_bankpop_bhf_TDlearn_Gradients(ptID,whichTrialsLM,false); % if plt true then this plots high gamma examples.
         end
...
%         if pt == 1
            % saving
%             save(fullfile(saveDir,'TDneuralFits_asymmetricPOPCTRL_RSTD.mat'),'TDdata','outcomeAndCueHGs','-v7.3')
%         end
