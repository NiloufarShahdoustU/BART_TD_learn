function [TDdataGradients,outcomeData,cueData,outcomeAndCueHGs] = BART_bankpop_bhf_TDlearn_Gradients(ptID,whichTrialsLM,plotFlag)   % BARTstats
...
%       include in the RSTD learning model.ning/
...
if length(nevList)>1
    warning('many nev files available for this patient. Using the first one in the list: %s', nevList(1).name);
end
...
balloonTimes = trigTimes(ismember(trigs,validBalloonTriggers));
...
% Make all trial-aligned vectors exactly the same length. This prevents
% trigger 4 / aborted-trial shifts from lowering model fits.
nTrials = min([length(outcomeTimes), length(balloonTimes), length(balloonIDs), length(outcomeType), length(BHV.data)]);
...
for tt = nTrials:-1:1
    ...
    % ATTENION:
    % the code above is wrong. we should go selectedchans not from 1 to
    % nChans, because nChans is only the length of selected chans

    LFPmat(:,:,tt)  = NSX.Data(selectedChans, floor(Fs*outcomeTimes(tt))-Fs*pre : floor(Fs*outcomeTimes(tt))+Fs*post);
    LFPmat1(:,:,tt) = NSX.Data(selectedChans, floor(Fs*balloonTimes(tt))-Fs*pre : floor(Fs*balloonTimes(tt))+Fs*post);
...
outcomeAndCueHGs.cueHG = HGmat_cue;
outcomeAndCueHGs.tSec = tSec;
outcomeAndCueHGs.timeWin = [-pre post];

%save as outcomeAndCueHGs here: (takes about 1.5 mins)
tic
...
% another window
startAnalysis_outcome = 0.25;
endAnalysis_outcome = 1.25;
...
TDdataGradients = TDlearn_rstd_Gradients(ptID,outcomeData,cueData,whichTrialsLM); % ,fileparts(parentDir) % rstd2
elapsedTDlearn = toc;
fprintf('TDlearn_rstd_Gradients.m took %.2f seconds to run...\n', elapsedTDlearn)
...
% Safely collect old ANOVA p-values plus the new DeltaAIC/LRT results.
nFits = length(TDdataGradients.neuralFit);
pRSTD_VE_cue = nan(1,nFits);
pRSTD_Color_cue = nan(1,nFits);
pRSTD_Type_cue = nan(1,nFits);
pRSTD_VE_cueSuccess = nan(1,nFits);
pRSTD_Color_cueSuccess = nan(1,nFits);
pRSTD_Type_cueSuccess = nan(1,nFits);
pRSTD_PE_outcome = nan(1,nFits);
pRSTD_Color_outcome = nan(1,nFits);
pRSTD_Type_outcome = nan(1,nFits);
pRSTD_Outcome_outcome = nan(1,nFits);
pRSTD_PE_outcomeSuccess = nan(1,nFits);
pRSTD_Color_outcomeSuccess = nan(1,nFits);
...
for cc = nFits:-1:1
    ...
    % missing rows/columns do not crash after changing model formulas.
    tmpA = TDdataGradients.neuralFit(cc).ANOVA_rstd_Cue;
    if istable(tmpA) && width(tmpA) >= 5
        if height(tmpA) >= 2; pRSTD_VE_cue(cc) = tmpA{2,5}; end
        if height(tmpA) >= 3; pRSTD_Color_cue(cc) = tmpA{3,5}; end
        if height(tmpA) >= 4; pRSTD_Type_cue(cc) = tmpA{4,5}; end
    end

    tmpA = TDdataGradients.neuralFit(cc).ANOVA_rstd_CueSuccess;
    if istable(tmpA) && width(tmpA) >= 5
        if height(tmpA) >= 2; pRSTD_VE_cueSuccess(cc) = tmpA{2,5}; end
        if height(tmpA) >= 3; pRSTD_Color_cueSuccess(cc) = tmpA{3,5}; end
        if height(tmpA) >= 4; pRSTD_Type_cueSuccess(cc) = tmpA{4,5}; end
    end

    tmpA = TDdataGradients.neuralFit(cc).ANOVA_rstd_Outcome;
    if istable(tmpA) && width(tmpA) >= 5
        if height(tmpA) >= 2; pRSTD_PE_outcome(cc) = tmpA{2,5}; end
        if height(tmpA) >= 3; pRSTD_Color_outcome(cc) = tmpA{3,5}; end
        if height(tmpA) >= 4; pRSTD_Type_outcome(cc) = tmpA{4,5}; end
        if height(tmpA) >= 5; pRSTD_Outcome_outcome(cc) = tmpA{5,5}; end
    end

    tmpA = TDdataGradients.neuralFit(cc).ANOVA_rstd_OutcomeSuccess;
    if istable(tmpA) && width(tmpA) >= 5
        if height(tmpA) >= 2; pRSTD_PE_outcomeSuccess(cc) = tmpA{2,5}; end
        if height(tmpA) >= 3; pRSTD_Color_outcomeSuccess(cc) = tmpA{3,5}; end
        if height(tmpA) >= 4; pRSTD_Type_outcomeSuccess(cc) = tmpA{4,5}; end
    end

    % New preferred significance: baseline model vs full latent-variable
    % model. DeltaAIC > 0 means the full model is better; pLRT < .05 means
    % the latent variable significantly improves fit over baseline.
    if isfield(TDdataGradients.neuralFit(cc),'deltaAIC_comparison') && ~isempty(TDdataGradients.neuralFit(cc).deltaAIC_comparison)
        tmpC = TDdataGradients.neuralFit(cc).deltaAIC_comparison;
    ...
    try
        TDdataGradients.neuralFit(cc).NMManatomicalLabel = deblank(anatomicalLocs{selectedChans(cc)});
    catch
...
% Keep this aligned with the already-trimmed nTrials. Do NOT drop the last
% element again here; doing so mismatches isActive/color/outcome indices.
balloonIDs = balloonIDs(1:nTrials);
isActive = balloonIDs < 10; % trials 11,12,13,14 are all controls

% Initialize balloonColors as a categorical array
balloonColors = strings(size(balloonIDs));
...
balloonColors = categorical(balloonColors);
...
resultsCell = {BHV.data(1:nTrials).result};
...
if plotFlag
    % (9) Plotting and looping through channels with significant DeltaAIC/LRT effects.
    for ch2 = nChans:-1:1
        if exist('sigDeltaAIC_any','var') && ch2 <= numel(sigDeltaAIC_any) && ~sigDeltaAIC_any(ch2)
            continue
        end

        % % plotting responses for variables with significant latent encoding.
        ...
        rLMve = fitlm(rR, rF);
        nTrialsPlot = length(resultsCell); % trial count for plotting already aligned above.
        % plotting
        ...
        % saving
        saveas(ch2,fullfile(saveDir,[ptID '_' deblank(trodeLabels_selectedChans{ch2}) '_' TDdataGradients.neuralFit(ch2).new_trodeLabel '_RSTD_exampleResponses_' smoothType '.pdf']))
        close(ch2)
